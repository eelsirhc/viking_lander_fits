...
def models(filename, startrow=0, stoprow=None, delimiter=None, nmodes=5):
...
def parameters(parameter_filename, viking_filename, suffix="low"):
    ...
    viking = core.convert_AP_to_SC(core.read_file(viking_filename))
    data = []
    for simulation in parameters.index:
        if simulation.endswith(suffix) or simulation == "base":
            df = core.convert_AP_to_SC(core.read_file("fit/{}.fit".format(simulation)))[
                viking.columns
            ]
