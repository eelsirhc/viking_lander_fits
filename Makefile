...
fit_data_low = $(patsubst %.data,%.fit,$(low))
fit_data_high = $(patsubst %.data,%.fit,$(high))
fit_baseline = $(patsubst %.data,%.fit,$(baseline))
...
viking_fit_low = $(lander).low.fit
viking_fit_high = $(lander).high.fit 
